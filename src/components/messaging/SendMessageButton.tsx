...
interface SendMessageButtonProps {
  ...
  postId?: number
  className?: string
  variant?: 'default' | 'small' | 'icon-only' | 'text-link'
  label?: string
}
...
const SendMessageButton: React.FC<SendMessageButtonProps> = ({
  ...
  if (variant === 'text-link') {
    return (
      <button
        onClick={handleSendMessage}
        className={`inline-flex items-center space-x-1 text-sm text-[var(--primary)] hover:opacity-80 transition-opacity ${className}`}
      >
        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
